...
func TestQP1(t *testing.T) {
	...
	// Constants
	solver := solvers.NewGurobiSolver() // Get New Gurobi Solver

	// Algorithm
	m := goop.NewModel()
	m.ShowLog(false)
	x := m.AddVar(-100, 100, goop.Continuous)
	y := m.AddVar(-100, 100, goop.Continuous)
	z := m.AddVar(-100, 100, goop.Continuous)
